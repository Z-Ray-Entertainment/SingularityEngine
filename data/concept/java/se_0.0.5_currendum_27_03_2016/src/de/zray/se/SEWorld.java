...
public class SEWorld {
    ...
    public SEWorld(){
        aiModule = new SEAIModule();
        glModule = new SEGLModule();
        bullettModule = new SEBulletModule();
        audioModule = new SEAudioModule();
    }
    ...
    private void pollKeyboardInput(){
        while(Keyboard.next()){
            if(Keyboard.getEventKeyState()){
                ...
                if(guis != null && guis.size() > currentGUI){
                    guis.get(currentGUI).keyTiped(key);
                }
            }
        }
        ...
        for(int i = 0; i < Keyboard.getKeyCount(); i++){
            if(Keyboard.isKeyDown(i)){
                ...
                if(guis != null && guis.size() > currentGUI){
                    guis.get(currentGUI).keyPressed(i);
                }
            }
        }
